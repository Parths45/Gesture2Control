...
class SystemController:
    ...
    def run(self):
        ...
        while self.running:
            ...
            if detection_result.hand_landmarks:
                ...
                # B. NORMAL MODE (Not Dragging)
                    ...
                    if state == "MOVE":
                        ...
                        # Use Index Tip (8) for more expressive scrolling than MCP
                        curr_scroll_y = hand_landmarks[8].y 
                        if hasattr(self, 'last_scroll_y'):
                            dy = curr_scroll_y - self.last_scroll_y
                            # Lower threshold for more sensitivity
                            if abs(dy) > 0.002: 
                                import pyautogui
                                # Add a multiplier for smoother but faster scrolling
                                scroll_amount = int(-dy * config.SCROLL_SPEED * 150)
            ...
            # 8. Display & IO
            # Check if window is visible/minimized to avoid LAG
            window_visible = True
            try:
                if cv2.getWindowProperty(config.APP_NAME, cv2.WND_PROP_VISIBLE) < 1:
            ...
            if window_visible:
                ...
                # If minimized, still need waitKey to process events
                key = cv2.waitKey(1) 
                # Keep CPU usage low if minimized and no hand detected
