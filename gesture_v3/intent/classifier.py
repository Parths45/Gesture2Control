...
class GestureClassifier:
    ...
    def process(self, landmarks):
        ...
        # C. PEACE (Scroll)
        # Condition: Index and Middle up, Ring down. Pinky doesn't matter much.
        is_peace = fingers_up[1] and fingers_up[2] and not fingers_up[3] 
        
        # D. PINCHES
        # Index Pinch (Left Click)
        dist_index = math.hypot(landmarks[4].x - landmarks[8].x, landmarks[4].y - landmarks[8].y)
        # Use a slightly larger threshold for classification than for confidence if we want stability,
        # but here we'll just use the config value.
        ...
        if is_pinch_index:
            ...
            # Maintain high confidence while pinched
